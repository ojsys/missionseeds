 ...
 *
 * This is the original Mission Seedlings landing page, preserved as-is. It
 * keeps its own settings keys (hero_*, seed_*, who_*, receive_*, harvest_*,
 * deadline_*), its growth-vine spine, and its inline editing exactly as they
 * shipped — the only change is that the site-wide header and footer now come
 * from includes/layout.php instead of being hard-coded here.
 *
 * Between application cycles, set Settings → Call status to "closed" (the page
 * stays up with a closed notice) or "hidden" (the page 404s for the public).
 * To run a new call, update the deadline and form URL and set it back to open.
 */
...
$meta['title']       = get_setting('hero_heading', 'Call for applications');
$meta['description'] = get_setting('hero_subheading');
$meta['body_class']  = 'page-call has-vine';
$meta['nav_active']  = 'call';
?>
      ...
      <div class="hero-copy">
        ...
        <div class="hero-actions">
          ...
          <a class="btn-primary" href="<?= e(url('/contact')) ?>">
            Register interest for the next cycle
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M5 12h14M13 6l6 6-6 6"/></svg>
          </a>
          <?php endif; ?>
      ...
      <div class="partners-grid">
        <?php foreach ($partners as $p): ?>
          ...
          <a class="partner-logo" href="<?= e($p['link_url']) ?>" target="_blank" rel="noopener noreferrer">
            <img src="<?= e(image_url($p['logo_path']) ?? '') ?>" alt="<?= e($p['name']) ?>" loading="lazy">
          </a>
          <?php else: ?>
  ...
  <section id="interest" class="harvest">
    <div class="section-inner reveal">
      ...
      <div class="deadline-card" style="margin-top:32px;">
        ...
        <?php else: ?>
        <div class="eyebrow">This call has closed</div>
        <div class="deadline-date"><?= e(format_deadline($deadlineDate)) ?></div>
        <p class="note"><?= editable('call_closed_note', $isAdmin, true) ?></p>
        <div class="harvest-actions">
          <a class="btn-apply" href="<?= e(url('/contact')) ?>">
            Register interest for the next cycle
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M5 12h14M13 6l6 6-6 6"/></svg>
          </a>
          <?php if ($whatsapp): ?>
